...
class Solution 
{
    ...
    public static void rec(ListNode node)
    {
        ...
        rec(node.next);
        if(node.val!=headOfLL.val)
        {
            ans=false;
            return ;
        }
        if(headOfLL.next!=null)
        {
    ...
    {
        // method1 : using recursion. Go till null, while coming back check for equality of current nodes with the moving head each time. If they are not equal then it is not a palindrome
        headOfLL=head;
        rec(head);
        return ans;
        
        //method2:2 pointers. slow goes 1 step at a time.fast goes 2 steps at a time. when fast==null we break. Now start will be at middle node (in case of odd length) or at last node of 1st half of 2 equal halves(in case of even length). Cut list into 2 parts at this position
        // Now if both parts are same then its a palindrome else not
